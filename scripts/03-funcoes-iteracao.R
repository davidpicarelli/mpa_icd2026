...
# usando o valor padrao de periodos (1 periodo)
...
resultado |>
  select(empresa, situacao, roa, roe, margem_liquida, endividamento)
...
# map_dbl: Exemplo 1
# Formato básico
resultado <- map_dbl(entrada, funcao_a_aplicar)

#Exemplo conceitual
# aplica a raiz quadrada a cada elemento do vetor
...
map_dbl(taxas, \(taxas) calcular_valor_futuro(10000, taxa, 10))
...
# Exemplo 10
# fluxo de caixa anuais do projeto
fluxos_projetos <- c(120000, 135000, 150000, 140000, 130000)
...
vpl_por_taxa <- map_dbl(
  taxas_desconto,
  \(taxa) calcular_vpl(500000, fluxos_projetos, taxa, 50000)
)
...
vpl_simulado <- map_dbl(
  1:5000,
  \(i) {
    #sorteia 5 fluxos, uma para cada ano do projeto
...
# Exemplo 11: Como a simulação é repetida?
